...
def create_eval_overview(out_file: str, experiments: list):
    ...
    # column names
    header = ['ID', 'Language', 'Features', 'Batch Size', 'Effective Batch Size', 'Test Batch Size', 'Max Epochs',
    ...
    with open(out_file, "w", encoding="utf-8") as result_file:
        ...
        for config_file in os.listdir('configs/parameter_tuning'):
            ...
            try:
                freq = int(config_file['update_freq'])
                bs = int(config_file['batch_size'])
                exp_row.append(str(freq*bs))
            except:
                exp_row.append(config_file['batch_size'])
...
if __name__=='__main__':
    ...
    if args["exps"]:
        create_eval_overview(args["out"], args["exps"])
    else:
