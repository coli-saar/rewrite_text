...
from prepare_word_embeddings_frequency_ranks import load_ranks
from feature_bin_preparation import create_bins
...
def maximum_dependency_length(s, t, lang):
...
def character_length_ratio(_src, _tgt):
...
def properties_word_freq_in_sentence(frequency_ranks):
    """
...
def word_frequency_rank(c, s, lang):
    """
    ...
    ranks = load_ranks(lang)  # TODO move this somewhere global
    ...
    nlp_model = spacy.load(lang_model[lang])
    doc_target = nlp_model(s)
    doc_source = nlp_model(c)
    ...
    target_ranks = [get_log_freq_rank_word(w.text, ranks) for w in doc_target if word_checks(w.text) and
                    stop_word_check(w.text, lang)]
    source_ranks = [get_log_freq_rank_word(w.text, ranks) for w in doc_source if word_checks(w.text) and
                    stop_word_check(w.text, lang)]
    third_quantile_target = properties_word_freq_in_sentence(target_ranks)
    third_quantile_source = properties_word_freq_in_sentence(source_ranks)
    return third_quantile_target / third_quantile_source


s = "A"
#t = "Today I got to school early."

t2 = "Heute bin ich früh in die Schule angekommen!"
t3 = "Heute fahre ich mit Fukufuji zur Schule ztvcbu."

#print(maximum_dependency_length(s, t, "en"))
#print(maximum_dependency_length(s, t2, "de"))

f1 = "ich war heute den ganzen Tag im Büro"
f2 = "Ist über die Butter nicht schon alles gesagt?"
f3 = "Russische Militärs benutzen in der Ukraine offenbar neben Streumunition auch Vakuumbomben."


# word_frequency_rank(s, f1.lower(), "de")
# word_frequency_rank(s, f2.lower(), "de")
# word_frequency_rank(s, f3.lower(), "de")


def feature_bundle_sentence(source, target, lang, ranks, list_of_desired_features):
    bundle = {}
    if "frequency" in list_of_desired_features:
        v = word_frequency_rank(source, target, lang)  # TODO: fix ranks: should be an arg
        bundle["frequency"] = v
    if "dependency" in list_of_desired_features:
        v = maximum_dependency_length(source, target, lang)
        bundle["dependency"] = v
    if "length" in list_of_desired_features:
        v = character_length_ratio(source, target)
        bundle["length"] = v
...
def feature_bins_bundle_sentence(source, target, lang, ranks, list_of_desired_features, feature_bin_dictionary):
    bundle = {}
    if "frequency" in list_of_desired_features:
        v = word_frequency_rank(source, target, lang)  # TODO: fix ranks: should be an arg
